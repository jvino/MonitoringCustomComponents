...
import ch.cern.alice.o2.kafka.utils.YamlNewAggregatorConfig;
...
public class NewAggregatorAvg {
    private static Logger logger = LoggerFactory.getLogger(NewAggregatorAvg.class); 
    ...
    private static String DEFAULT_CLIENT_DESCRIPTION = "This tool is used to aggregate using the average function.";
	...
	*/
	public static Boolean filterRecords(String mfKey, String tvtValue){
	...
	public static List<KeyValue<String, Double>> manageTagsAndValue( String mfKey, String tvtValue){
		if( statsEnabled ) {
			try {
				stats();
			} catch (IOException e) {
	...
	public static void main(String[] args) throws Exception {
		...
		YamlNewAggregatorConfig config = mapper.readValue( new File(config_filename), YamlNewAggregatorConfig.class);
        ...
        List<Map<String,String>> selectionConfig = config.getSelection();
        Map<String,String> statsConfig = config.getStats_config();
		for ( Map<String,String> entry: selectionConfig){
		...
		allowedFieldMeas = tagsToRemove.keySet();
		logger.info("selection.meas-field: " + allowedFieldMeas);
		logger.info("selection.'tags to remove': " + tagsToRemove);
        ...
        try {
			KStream<String, String> avg_data = builder.stream(input_topic);
			KStream<String, String> avg_aggr_stream = avg_data
					.filter( (key,value) -> filterRecords(key,value))
					.flatMap((key,value) -> manageTagsAndValue(key,value))
