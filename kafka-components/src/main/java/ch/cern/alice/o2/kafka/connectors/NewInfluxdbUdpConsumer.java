...
public class NewInfluxdbUdpConsumer {
	private static String version = "0.1.1";
	/*
	Version 0.1.1 Variable overflow managing

	*/
	private static Logger logger = LoggerFactory.getLogger(NewInfluxdbUdpConsumer.class); 
	...
	private static void sendUdpData(byte[] key, byte[] value) {
		byte [] data2send = convertData(key,value);if (data2send.length > 0){
			try {
